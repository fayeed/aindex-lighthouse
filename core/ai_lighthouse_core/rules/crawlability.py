    ...
    if response is None or response.status_code >= 400:
      return [
        Issue(
          ...
          data={"robots_url": robots_url, "status_code": response.status_code if response else "No Response"}
        )
      ]
  ...
  async def run(self, html: str, url: str, soup) -> list[BaseRule.Issue]:
    ...
    return [
      Issue(
        ...
        data={"canonical_url": canonical_url, "hreflangs": [h.get("href") for h in hreflangs]}
      )
    ]
    ...
    if response is None or response.status_code >= 400:
      return [
        Issue(
          ...
          data={"canonical_url": canonical_url, "status_code": response.status_code if response else "No Response"}
        )
      ]
...
class SitemapReachable(BaseRule):
  ...
  async def run(self, html: str, url: str, soup) -> list[BaseRule.Issue]:
    ...
    sitemaps = [line.split(":", 1)[1].strip() for line in txt.splitlines() if line.startswith("sitemap:")]
    issues = []
      ...
      if response is None or response.status_code >= 400:
        issues.append(
          Issue(
            ...
            data={"sitemap_url": sm_url, "status_code": response.status_code if response else "No Response"}
          )
        )
...
class CrawlAllowed(BaseRule):
  ...
  async def run(self, html: str, url: str, soup) -> list[BaseRule.Issue]:
    ...
    disallow_lines = [line.split(":", 1)[1].strip() for line in txt.splitlines() if line.startswith("disallow:")]
